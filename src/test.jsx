...
async function ai(prompt, sys='You are EduAI, an expert educational assistant for university professors. Be detailed, practical, and professional in your responses.') {
  const p = String(prompt || '').toLowerCase();
  const s = String(sys || '').toLowerCase();

  if (p.includes('quiz') || p.includes('assessment') || p.includes('question')) {
    return 'Here is a concise assessment plan: define learning outcomes, keep questions aligned to Bloom’s taxonomy, mix objective and short-answer items, and include a short rubric for grading.';
  }

  if (p.includes('lecture') || p.includes('lesson') || p.includes('class')) {
    return 'Suggested lecture structure: warm-up, concept explanation, worked example, guided practice, and a 3-minute exit check.';
  }

  if (p.includes('feedback') || p.includes('review')) {
    return 'Feedback suggestion: start with strengths, identify one priority improvement area, and end with a clear next step the student can apply immediately.';
  }

  if (p.includes('course') || p.includes('curriculum') || p.includes('syllabus')) {
    return 'Curriculum suggestion: define outcomes first, map topics week by week, then attach assessments and readings to each outcome.';
  }

  if (p.includes('project') || p.includes('research')) {
    return 'Project suggestion: focus on a clear problem statement, a manageable methodology, measurable outcomes, and a simple evaluation plan.';
  }

  return 'Demo response: this version runs fully as a static website. For live AI responses, connect a secure backend API later.';
}
...
function AIBox({title,placeholder,buildPrompt,resultLabel='AI Response'}) {
  const [inp,setInp]=useState('');const [res,setRes]=useState('');const [load,setLoad]=useState(false);
  async function run(){setLoad(true);setRes('');const r=await ai(buildPrompt(inp));setRes(r);setLoad(false);}
...
function CapstoneSubmissionCenter({submissions,setSubmissions}) {
  ...
  async function onPickFile(file){ if(!file) return; const fileUrl=await fileToDataUrl(file); setForm(prev=>({...prev,file,fileName:file.name,fileUrl,fileType:fileKind(file)})); }
