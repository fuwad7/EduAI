...
const SUPABASE_BUCKET = 'teacher-files';
...
async function persistFile(file, folder = SUPABASE_BUCKET) {
  ...
  return { fileUrl: await fileToDataUrl(file), storagePath: '' };
}
...
function AIBox({title,placeholder,buildPrompt,resultLabel='AI Response'}) {
  const [inp,setInp]=useState('');const [res,setRes]=useState('');const [load,setLoad]=useState(false);const [copied,setCopied]=useState(false);
  async function run(){setLoad(true);setCopied(false);setRes('');const r=await ai(buildPrompt(inp));setRes(r);setLoad(false);}
  ...
  return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
    ...
    {res&&<div style={{marginTop:14,background:C.card,borderRadius:8,padding:16,color:C.sub,fontSize:13,lineHeight:1.7,whiteSpace:'pre-wrap',maxHeight:350,overflowY:'auto',border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.accent}`}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,marginBottom:8}}>
        ...
        <button onClick={copyResult} style={{display:'inline-flex',alignItems:'center',gap:6,border:`1px solid ${C.border}`,background:'transparent',color:C.text,borderRadius:8,padding:'6px 10px',fontSize:11,cursor:'pointer'}}>
          <Copy size={12}/>
          {copied ? 'Copied' : 'Copy'}
        </button>
      </div>
...
function LoginPage({onLogin}) {
  ...
  async function handleSubmit(){
    if(!email||!pass||(mode==='signup'&&!name.trim())){setErr('Please fill all required fields');return;}
    setErr('');setLoad(true);
      ...
      if (isSupabaseReady()) {
        if (mode === 'signup') {
          ...
          if (sessionUser) {
            onLogin({
              id: sessionUser.id,
              name: name || sessionUser.email || 'Teacher',
          ...
          onLogin({
            ...
            name: sessionUser?.user_metadata?.full_name || sessionUser?.email || email,
            email: sessionUser?.email || email,
          });
        }
      } else {
        if(email==='teacher@eduai.com'&&pass==='demo123'){
          onLogin({ name:'Dr. Sarah Ahmed', email:'teacher@eduai.com', id:'demo-teacher' });
        } else {
          setErr('Invalid credentials. Use the demo account or connect Supabase.');
        }
      }
    } catch (error) {
    ...
    <div style={{width:420,position:'relative',zIndex:1}}>
      ...
      <Card style={{padding:32}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:6}}>
          <h2 style={{fontSize:20,fontWeight:700,color:C.text,fontFamily:'"Playfair Display",serif'}}>{mode==='signup' ? 'Create Teacher Account' : 'Teacher Login'}</h2>
          <Btn onClick={()=>setMode(mode==='signup' ? 'signin' : 'signup')} variant="ghost" size="sm" icon={mode==='signup' ? LogIn : UserPlus}>
            {mode==='signup' ? 'Sign In' : 'Create Account'}
          </Btn>
        </div>
        <p style={{color:C.sub,fontSize:13,marginBottom:24}}>{mode==='signup' ? 'Create a password-protected teacher account' : 'Sign in to access your dashboard'}</p>
        {mode==='signup' && <Input label="Full Name" value={name} onChange={setName} placeholder="Dr. Sarah Ahmed"/>}
        ...
        <Btn onClick={handleSubmit} full disabled={load} size="lg">{load?(mode==='signup'?'Creating account...':'Signing in...'):(mode==='signup'?'Create Teacher Account':'Sign In to EduAI')}</Btn>
        <div style={{marginTop:20,background:C.surface,borderRadius:8,padding:14,border:`1px dashed ${C.border}`}}>
          <div style={{fontSize:11,color:C.accent,fontWeight:600,marginBottom:6}}>FREE BACKEND OPTION</div>
          <div style={{fontSize:12,color:C.sub,lineHeight:1.5}}>
            Use <strong style={{color:C.text}}>Supabase</strong> for teacher accounts, passwords, and file storage.
          </div>
          <div style={{fontSize:11,color:C.muted,marginTop:8}}>Demo login: teacher@eduai.com / demo123</div>
          <Btn onClick={()=>{setMode('signin');setEmail('teacher@eduai.com');setPass('demo123');}} variant="ghost" size="sm" style={{marginTop:8}}>Use Demo Account</Btn>
        </div>
      </Card>
    </div>
  </div>;
...
function CurriculumModule({courses,setCourses,curriculumAssets,setCurriculumAssets}) {
  ...
  return <div><SectionHead title="Curriculum Design & Update" sub="Manage learning outcomes, syllabi, curriculum files, course documents, and program files"/><TabBar tabs={tabs} active={tab} setActive={setTab}/>
    {tab==='mapping'&&<CLOPLOMapper courses={courses}/>}
    {tab==='syllabus'&&<SyllabusManager courses={courses} setCourses={setCourses}/>}
    {tab==='repository'&&<CurriculumRepository courses={courses} curriculumAssets={curriculumAssets} setCurriculumAssets={setCurriculumAssets}/>}
    {tab==='gap'&&<GapAnalyzer/>}
    {tab==='trends'&&<TrendAnalyzer/>}
  </div>;
}

function CurriculumRepository({courses,curriculumAssets,setCurriculumAssets}) {
  const [modal,setModal]=useState(false);
  const [viewer,setViewer]=useState(null);
  const [selectedCourse,setSelectedCourse]=useState('all');
  const [form,setForm]=useState({title:'',category:'Curriculum Document',notes:'',file:null,fileName:'',fileUrl:'',fileType:''});
  ...
  function resetForm(){ setForm({title:'',category:'Curriculum Document',notes:'',file:null,fileName:'',fileUrl:'',fileType:''}); }
  async function onPickFile(file){
    if(!file) return;
    const { fileUrl } = await persistFile(file, 'teacher-resources');
    setForm(prev=>({...prev,file,fileName:file.name,fileUrl,fileType:fileKind(file)}));
  }
  function add(){
...
function ResourceLib({materials,setMaterials}) {
  const [modal,setModal]=useState(false);
  const [viewer,setViewer]=useState(null);
  const [form,setForm]=useState({title:'',course:'',type:'Lecture Notes',date:'',notes:'',file:null,fileName:'',fileUrl:'',fileType:''});
  ...
  return <div>
    ...
    <Modal open={modal} onClose={()=>{setModal(false);resetForm();}} title="Add Teaching Resource" width={560}>
      ...
      <div style={{marginBottom:14}}>
        <label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Upload File (PDF / PPT / PPTX / DOC / DOCX)</label>
        <input
          type="file"
          accept=".pdf,.ppt,.pptx,.doc,.docx,application/pdf,application/msword,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
...
function CapstoneSubmissionCenter({submissions,setSubmissions}) {
  ...
  const [form,setForm]=useState({student:'',title:'',supervisor:'',abstract:'',file:null,fileName:'',fileUrl:'',fileType:'',notes:'',report:'',provider:'groq'});
  async function onPickFile(file){ if(!file) return; const { fileUrl } = await persistFile(file, 'capstone-uploads'); setForm(prev=>({...prev,file,fileName:file.name,fileUrl,fileType:fileKind(file)})); }
  function resetForm(){ setForm({student:'',title:'',supervisor:'',abstract:'',file:null,fileName:'',fileUrl:'',fileType:'',notes:'',report:'',provider:'groq'}); }
  ...
  return <div>
    ...
    <Modal open={modal} onClose={()=>{setModal(false);resetForm();}} title="Upload Student Capstone Project" width={700}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
        <div><label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Upload Project File (PDF, PPT, PPTX, DOC, DOCX)</label><input type="file" accept=".pdf,.ppt,.pptx,.doc,.docx,application/pdf,application/msword,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={e=>onPickFile(e.target.files?.[0])} style={{width:'100%',color:C.text,fontSize:13}}/></div>
...
export default function EduAI() {
  ...
  useEffect(()=>{(async()=>{
    ...
    const ca=await gget('eduai_curriculum_assets',[]); const cs=await gget('eduai_capstone_submissions',[]);
    ...
    if (supabase) {
      ...
      if (sessionUser) {
        setUser({
          id: sessionUser.id,
          name: sessionUser.user_metadata?.full_name || sessionUser.email || 'Teacher',
  ...
  if(page==='login')return <LoginPage onLogin={(account)=>{setUser(account);setPage('app');}}/>;
