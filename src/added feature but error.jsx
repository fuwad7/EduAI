...
function ResourceLib({materials,setMaterials}) {
  const [modal,setModal]=useState(false);const [form,setForm]=useState({title:'',course:'',type:'Lecture Notes',date:'',notes:''});
  const types=['Lecture Slides','Lecture Notes','Reading List','Lab Manual','Assignment Brief','Video Link','External Resource'];
  function add(){setMaterials([...materials,{...form,id:Date.now(),size:'N/A',date:form.date||new Date().toISOString().split('T')[0]}]);setModal(false);setForm({title:'',course:'',type:'Lecture Notes',date:'',notes:''});}
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'flex-end',marginBottom:16}}><Btn onClick={()=>setModal(true)} icon={Plus}>Add Resource</Btn></div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
      {materials.map(m=><Card key={m.id} style={{padding:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}><Tag label={m.type} color={typeColor[m.type]||C.muted}/><button onClick={()=>del(m.id)} style={{background:'none',border:'none',color:C.red,cursor:'pointer'}}><Trash2 size={12}/></button></div>
    ...
    <Modal open={modal} onClose={()=>setModal(false)} title="Add Teaching Resource" width={460}>
      ...
      <div style={{marginBottom:14}}><label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Resource Type</label><select value={form.type} onChange={e=>setForm({...form,type:e.target.value})} style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'inherit',outline:'none'}}>{types.map(t=><option key={t}>{t}</option>)}</select></div>
      <Input label="Date" value={form.date} onChange={v=>setForm({...form,date:v})} type="date"/>
      <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><Btn onClick={()=>setModal(false)} variant="ghost">Cancel</Btn><Btn onClick={add} icon={Plus}>Add Resource</Btn></div>
    </Modal>
  </div>;
}
