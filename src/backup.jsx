  ...
  return (
    <div style={{background:C.surface, borderBottom: `1px solid ${C.border}`, padding:'16px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0}}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:20}}>
        ...
        <div style={{position:'relative', cursor:'pointer'}}> <Bell size={20} color={C.sub}/> <span style={{position:'absolute', top:-2, right:-2, width:10, height:10, borderRadius:'50%', background:C.red, border:`2px solid ${C.surface}`}}/> </div>
    ...
    <div className="fade-in">
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:32}}>
        ...
        <div>
          ...
          {courses.map(c => (
            <Card key={c.id} style={{marginBottom:12, padding:18}}>
...
function TeachingModule({grades, setGrades, courses}) {
  ...
  const tabs = [
    ...
    {id:'grader', icon:CheckSquare, label:'AI Grader'},
    {id:'feedback', icon:MessageCircle, label:'Feedback Generator'},
    {id:'analytics', icon:TrendingUp, label:'Learning Analytics'}
  ];
  return (
    <div className="fade-in"> 
      <SectionHead title="Teaching & Assessment" sub="Manage evaluations, grades, and AI-powered tools."/> 
...
function GradeBook({grades, setGrades}) {
  ...
  return (
    <div>
      <div style={{display:'flex', justifyContent:'flex-end', marginBottom:20}}> <Btn onClick={openAdd} icon={Plus}>Add Student</Btn> </div>
...
function CurriculumRepository({courses, setCourses, curriculumAssets, setCurriculumAssets}) {
  ...
  return (
    <div>
      ...
      {(cloExtracting || cloExtractMsg) && (
        <div style={{marginBottom:16, padding:'12px 16px', borderRadius:8, fontSize:13, display:'flex', alignItems:'center', gap:10, background:cloExtractMsg.startsWith('✓') ? `${C.green}18` : cloExtractMsg.includes('failed') ? `${C.red}18` : `${C.accent}18`, border: `1px solid ${cloExtractMsg.startsWith('✓') ? C.green + '44' : cloExtractMsg.includes('failed') ? C.red + '44' : C.accent + '44'}`, color:cloExtractMsg.startsWith('✓') ? C.green : cloExtractMsg.includes('failed') ? C.red : C.accent}}>
          {cloExtracting && <Loader size={14}/>}{cloExtractMsg}
        </div>
      )}
...
function CLOPLOMapper({courses, setCourses}) {
  ...
  async function handlePdfUpload(file) {
    if (!file) return;
    setError(''); setPdfFile(file); setExtractedCLOs([]); setCloMappings({}); setMappingDone(false); setLoading(true); setStep('Extracting text from PDF...');
    try {
      ...
      setPdfText(text); setStep('Analyzing CLOs with AI...');
      ...
      setExtractedCLOs(clos); setStep('Mapping CLOs to PLOs...');
  ...
  return (
    <div>
      ...
      <Card style={{marginBottom:24, border:`1px dashed ${C.accent}44`, background:`${C.accent}08`}}>
        <div style={{display:'flex', alignItems:'center', gap:12, marginBottom:16}}>
          <div style={{background:`${C.accent}22`, borderRadius:8, padding:10, display:'flex'}}> <Upload size={20} color={C.accent}/> </div>
          <div>
            <h3 style={{fontSize:16, fontWeight:700, color:C.text, fontFamily:FONT_SERIF}}>Upload CLO Document (PDF)</h3>
            <p style={{fontSize:12, color:C.muted, marginTop:4}}>Upload a PDF containing Course Learning Outcomes. AI will extract CLOs and auto-map each to the most relevant PLO.</p>
          </div>
        </div>
        <div style={{display:'flex', alignItems:'center', gap:16, flexWrap:'wrap'}}>
          <label style={{display:'inline-flex', alignItems:'center', gap:8, padding:'12px 20px', background:C.accent, color:'#fff', borderRadius:8, fontWeight:600, fontSize:14, cursor:loading ? 'not-allowed' : 'pointer', opacity:loading ? 0.5 : 1}}>
            <Upload size={16}/>{loading ? 'Processing...' : 'Choose PDF File'}
          ...
          {pdfFile && <span style={{fontSize:13, color:C.sub}}>{pdfFile.name}</span>}
          {loading && <span style={{fontSize:13, color:C.accent, display:'flex', alignItems:'center', gap:8}}> <Loader size={14} color={C.accent}/>{step}</span>}
        </div>
        {error && <div style={{marginTop:16, background:`${C.red}18`, border:`1px solid ${C.red}44`, borderRadius:8, padding:'12px 16px', color:C.red, fontSize:13}}>{error}</div>}
      </Card>

      {extractedCLOs.length > 0 && (
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:24}}>
          <Card>
            <h3 style={{fontSize:16, fontWeight:600, color:C.text, marginBottom:6, fontFamily:FONT_SERIF}}>Extracted CLOs from PDF</h3>
            <p style={{fontSize:12, color:C.muted, marginBottom:16}}>{extractedCLOs.length} CLOs found in "{pdfFile?.name}"</p>
            <div style={{maxHeight:420, overflowY:'auto'}}>
              {extractedCLOs.map((clo, i) => (
                <div key={i} style={{padding:'12px 14px', background:C.surface, borderRadius:8, marginBottom:10, fontSize:13, color:C.text, border:`1px solid ${C.border}`, borderLeft:`4px solid ${C.accent}`}}>
          ...
          <Card>
            <h3 style={{fontSize:16, fontWeight:600, color:C.text, marginBottom:6, fontFamily:FONT_SERIF}}>Program Outcomes (PO1–PO12)</h3>
            <p style={{fontSize:12, color:C.muted, marginBottom:16}}>Green = mapped by at least one CLO</p>
            <div style={{maxHeight:420, overflowY:'auto'}}>
              {PLOS.map(plo => {
                const mapped = mappedPLOIds.has(plo.id);
                return (
                  <div key={plo.id} style={{padding:'12px 14px', background:mapped ? `${C.green}15` : C.surface, borderRadius:8, marginBottom:10, fontSize:12, color:mapped ? C.text : C.muted, border:`1px solid ${mapped ? C.green + '44' : C.border}`, display:'flex', gap:10, alignItems:'flex-start'}}>
                    ...
                    {mapped && <CheckCircle size={14} color={C.green} style={{flexShrink:0, marginTop:2}}/>}
                  </div>
                );
              })}
            </div>
        ...
        <Card style={{marginBottom:24}}>
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{borderCollapse:'collapse', fontSize:12, width:'100%'}}>
              <thead> 
                <tr>
                  <th style={{padding:'10px 14px', textAlign:'left', color:C.muted, borderBottom:`1px solid ${C.border}`, minWidth:100, position:'sticky', left:0, background:C.card, zIndex:1}}>CLO</th>
                  {PLOS.map(p => <th key={p.id} style={{padding:'10px 8px', color:C.muted, borderBottom:`1px solid ${C.border}`, textAlign:'center', whiteSpace:'nowrap', fontSize:11}}>{p.id}</th>)}
                </tr> 
              </thead>
              <tbody>
                {extractedCLOs.map((clo, ci) => (
                  <tr key={ci} style={{borderBottom:`1px solid ${C.border}`, background:ci % 2 === 0 ? C.card : C.surface}}>
                    <td style={{padding:'10px 14px', color:C.text, fontWeight:500, position:'sticky', left:0, background:ci % 2 === 0 ? C.card : C.surface, zIndex:1, whiteSpace:'nowrap'}} title={clo.description}>{clo.id}</td>
                    {PLOS.map(p => {
                      const mapped = cloMappings[clo.id] === p.id;
                      return (
                        <td key={p.id} style={{textAlign:'center', padding:'10px 8px'}}>
                          {mapped ? (
                            <div style={{width:24, height:24, background:C.green, borderRadius:6, display:'inline-flex', alignItems:'center', justifyContent:'center'}}> <CheckCircle size={14} color="#fff"/> </div>
                          ) : (
                            <div style={{width:24, height:24, background:C.border, borderRadius:6, display:'inline-block', opacity:0.3}}/>
                          )}
                        </td>
                      );
                    })}
            ...
            {extractedCLOs.map(clo => {
              ...
              return (
                <div key={clo.id} style={{padding:16, background:C.surface, borderRadius:10, border:`1px solid ${C.border}`}}>
                  ...
                  <div style={{fontSize:12, color:C.sub, lineHeight:1.6, marginBottom:8}}> <strong style={{color:C.text}}>CLO: </strong> {clo.description}</div>
                  {po && <div style={{fontSize:12, color:C.muted, lineHeight:1.6}}> <strong style={{color:C.sub}}>PLO: </strong> {po.desc.length > 150 ? po.desc.substring(0,150) + '...' : po.desc}</div>}
                </div>
              );
            })}
          </div>
      ...
      {course && !mappingDone && extractedCLOs.length === 0 && (
        <div>
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:24}}>
            <Card>
              ...
              {(course.clos || []).length > 0 ? (course.clos || []).map((clo, i) => {
                ...
                <div style={{padding:20, background:C.surface, borderRadius:8, border:`1px dashed ${C.border}`, textAlign:'center'}}>
                  ...
                  <p style={{fontSize:12, color:C.muted}}>Upload a CLO PDF above or go to Course & Program Uploads to add a CLO Document</p>
                </div>
              )}
            </Card>
            <Card>
          ...
          <Card> 
            ...
            {editing ? (
              ...
              <div style={{fontSize:13, color:C.sub, display:'flex', flexDirection:'column', gap:10}}> 
                ...
                {course.cloDocument && <div> <span style={{color:C.muted}}>CLO Document: </span> <strong style={{color:C.green}}>{course.cloDocument}</strong> </div>} 
              </div>
            )}
          </Card>
          <Card> 
            ...
            {courseClos.length > 0 ? courseClos.map((clo, i) => {
              ...
              <div style={{padding:20, background:C.surface, borderRadius:8, border: `1px dashed ${C.border}`, textAlign:'center'}}>
                <p style={{fontSize:13, color:C.muted, marginBottom:6}}>No CLOs uploaded yet</p>
                <p style={{fontSize:12, color:C.muted}}>Go to CLO-PLO Mapping tab to upload a CLO document for this course</p>
              </div>
            )}
          </Card>
          <Card style={{gridColumn:'span 2'}}> 
...
export default function EduAI() {
  ...
  const [profileModal, setProfileModal] = useState(false); // CRITICAL STATE ADDED
  ...
  function handleProfileUpdate(newData) {
    ...
    // 4. CRITICAL: Close the modal automatically after saving!
    ...
    <div style={{display:'flex', height:'100vh', background:C.bg, color:C.text, fontFamily:'"DM Sans",system-ui,sans-serif', overflow:'hidden'}}>
      ...
      <div style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden'}}>
        ...
        <main style={{flex:1, overflowY:'auto', padding:'24px 28px'}}>
          ...
          {module === 'capstone' && <CapstoneModule projects={projects} setProjects={updProjects} capstoneSubmissions={capstoneSubmissions} setSubmissions={updCapstoneSubmissions}/>}
        </main>
      </div>
