...
async function ai(prompt, sys='You are EduAI, an expert educational assistant for university professors. Be detailed, practical, and professional in your responses.') {
  try {
    ...
    const d = await r.json(); return d.content?.[0]?.text || "Error generating response.";
  } catch { return "Network error. Please try again."; }
}
async function gset(key,val){try{await window.storage.set(key,JSON.stringify(val));}catch{}}
async function gget(key,fb){try{const r=await window.storage.get(key);return r?JSON.parse(r.value):fb;}catch{return fb;}}
...
function Btn({children,onClick,variant='primary',disabled,size='md',icon:Icon,full}) {
  ...
  return <button onClick={onClick} disabled={disabled} style={{...base,...sz[size],...vars[variant],width:full?'100%':'auto'}}>{Icon&&<Icon size={size==='sm'?12:14}/>}{children}</button>;
}
      ...
      {tabs.map((t) => (
        <button
          ...
          style={{
            ...
            color: active === t.id ? C.accent : C.sub,
...
// ===================== CURRICULUM MODULE =====================
function CurriculumModule({ courses, setCourses, resources, setResources }) {
  const [tab, setTab] = useState('mapping');
  const tabs = [
    { id: 'mapping', icon: Map, label: 'CLO-PLO Mapping' },
    { id: 'syllabus', icon: FileText, label: 'Syllabus Manager' },
    { id: 'repository', icon: Upload, label: 'Curriculum Uploads' },
  ...
  return (
    <div>
      <SectionHead
        title="Curriculum Design & Update"
        sub="Manage learning outcomes, syllabi, curriculum documents, and alignment resources"
      />
      ...
      {tab === 'repository' && <CurriculumRepository courses={courses} resources={resources} setResources={setResources} />}
      {tab === 'gap' && <GapAnalyzer />}
...
function CurriculumRepository({ courses, resources, setResources }) {
  ...
  const categories = ['Curriculum', 'CLO Document', 'PLO Document', 'CLO-PLO Mapping', 'Syllabus', 'Reference'];
  const accepted = '.pdf,.ppt,.pptx,.doc,.docx,.xls,.xlsx,.txt';
  ...
  async function addResource() {
    ...
    const dataUrl = await toDataURL(form.file);
    const next = [
      {
        id: Date.now(),
        ...
        fileType: form.file.type || form.file.name.split('.').pop().toUpperCase(),
  ...
  const previewIsPDF = preview && (
    preview.fileType === 'application/pdf' ||
    preview.fileName?.toLowerCase().endsWith('.pdf')
  );

  const previewIsOffice = preview && (
    preview.fileName?.toLowerCase().endsWith('.ppt') ||
    preview.fileName?.toLowerCase().endsWith('.pptx') ||
    preview.fileName?.toLowerCase().endsWith('.doc') ||
    preview.fileName?.toLowerCase().endsWith('.docx') ||
    preview.fileName?.toLowerCase().endsWith('.xls') ||
    preview.fileName?.toLowerCase().endsWith('.xlsx')
  );

  return (
    <div>
      <Card style={{ marginBottom: 18, borderLeft: `4px solid ${C.accent}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 6 }}>Upload curriculum and outcome files</h3>
            <p style={{ fontSize: 12, color: C.sub, lineHeight: 1.6, maxWidth: 820 }}>
              Teachers can upload curriculum documents, CLO sheets, PLO sheets, and CLO-PLO mapping files here.
              PDF files preview inline. PPT/PPTX, DOC/DOCX, and spreadsheet files can be opened from the repository and downloaded if the browser cannot preview them natively.
            </p>
          </div>
          <Btn onClick={() => setModal(true)} icon={Upload}>Add Curriculum Resource</Btn>
        </div>
      </Card>
        ...
        {resources.map((r) => (
          <Card key={r.id} style={{ padding: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10, gap: 8 }}>
              <Tag label={r.category} color={C.accent} />
              <button
            ...
            <h4 style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 6, lineHeight: 1.35 }}>{r.title}</h4>
            <div style={{ fontSize: 11, color: C.sub }}>{r.course}</div>
            <div style={{ fontSize: 11, color: C.sub, marginTop: 4 }}>{r.fileName}</div>
            <div style={{ fontSize: 11, color: C.sub, marginTop: 4 }}>Added: {r.added} • {r.size}</div>

            <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              <Btn size="sm" variant="blue" icon={Eye} onClick={() => setPreview(r)}>View</Btn>
              <a href={r.dataUrl} download={r.fileName} style={{ textDecoration: 'none' }}>
                <Btn size="sm" variant="ghost" icon={Save}>Download</Btn>
              </a>
            </div>
          </Card>
        ))}
      ...
      {resources.length === 0 && (
        <Card style={{ marginTop: 14 }}>
          <div style={{ fontSize: 13, color: C.sub }}>No curriculum resources uploaded yet.</div>
        </Card>
      )}

      <Modal open={modal} onClose={() => setModal(false)} title="Add Curriculum Resource" width={520}>
        <Input label="Resource Title" value={form.title} onChange={(v) => setForm({ ...form, title: v })} placeholder="e.g. BSc CSE Curriculum 2025" />
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, color: C.sub, marginBottom: 6, fontWeight: 600 }}>Course / Program</label>
          <select
            value={form.course}
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, color: C.sub, marginBottom: 6, fontWeight: 600 }}>Document Type</label>
          <select
            value={form.category}
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, color: C.sub, marginBottom: 6, fontWeight: 600 }}>File Upload</label>
          <input
            type="file"
            accept={accepted}
            ...
            style={{ width: '100%', background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 12px', color: C.text, fontSize: 13, fontFamily: 'inherit', outline: 'none' }}
          />
          <div style={{ fontSize: 11, color: C.muted, marginTop: 6 }}>Accepted: PDF, PPT/PPTX, DOC/DOCX, XLS/XLSX, TXT</div>
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea
            ...
            style={{ width: '100%', minHeight: 90, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 12px', color: C.text, fontSize: 13, fontFamily: 'inherit', outline: 'none', resize: 'vertical' }}
          />
        </div>

        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <Btn onClick={() => setModal(false)} variant="ghost">Cancel</Btn>
          <Btn onClick={addResource} icon={Upload}>Save Resource</Btn>
        </div>
      </Modal>

      <Modal open={!!preview} onClose={() => setPreview(null)} title={preview?.title || 'Preview'} width={900}>
        {preview && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 12, color: C.muted, marginBottom: 4 }}>{preview.category} • {preview.course}</div>
                <div style={{ fontSize: 14, color: C.text, fontWeight: 700 }}>{preview.fileName}</div>
              </div>
              <a href={preview.dataUrl} download={preview.fileName} style={{ textDecoration: 'none' }}>
                <Btn size="sm" variant="ghost" icon={Save}>Download</Btn>
              </a>
            </div>
            ...
            {previewIsPDF ? (
              <iframe
                src={preview.dataUrl}
                title={preview.title}
                style={{ width: '100%', height: '70vh', border: `1px solid ${C.border}`, borderRadius: 12, background: '#fff' }}
              />
            ) : (
              <div style={{ border: `1px solid ${C.border}`, borderRadius: 12, padding: 18, background: C.surface }}>
                <div style={{ fontSize: 13, color: C.text, lineHeight: 1.7, marginBottom: 10 }}>
                  A native preview is not always available for this file type in the browser.
                  Use the download button to open it in PowerPoint, Word, Excel, or a compatible viewer.
                </div>
                <div style={{ display: 'grid', gap: 8, gridTemplateColumns: 'repeat(2,minmax(0,1fr))' }}>
                  <div style={{ fontSize: 12, color: C.sub }}>Resource type: {preview.category}</div>
                  <div style={{ fontSize: 12, color: C.sub }}>Added: {preview.added}</div>
                  <div style={{ fontSize: 12, color: C.sub }}>Size: {preview.size}</div>
                  <div style={{ fontSize: 12, color: C.sub }}>File: {preview.fileName}</div>
                </div>
                {previewIsOffice && (
                  <div style={{ marginTop: 14, fontSize: 12, color: C.accent }}>
                    This file is an Office document. Use Download to open it in PowerPoint or another compatible app.
                  </div>
                )}
              </div>
            )}
...
export default function EduAI() {
  ...
  useEffect(()=>{(async()=>{
    ...
    const cr=await gget('eduai_curriculum_resources',[]);
    setGrades(g);setCourses(c);setProjects(p);setMaterials(m);setCurriculumResources(cr);
  })();},[]);
  ...
  async function updCurriculumResources(cr){setCurriculumResources(cr);await gset('eduai_curriculum_resources',cr);}
  ...
  return <div style={{display:'flex',height:'100vh',background:C.bg,color:C.text,fontFamily:'"DM Sans",system-ui,sans-serif',overflow:'hidden'}}>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <main style={{flex:1,overflowY:'auto',padding:'24px 28px'}}>
        ...
        {module==='curriculum'&&<CurriculumModule courses={courses} setCourses={updCourses} resources={curriculumResources} setResources={updCurriculumResources}/>}
