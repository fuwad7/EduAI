...
function getDisplayName(user) {
  return user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split('@')?.[0] || 'Teacher';
}
  ...
  return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
    ...
    {res&&<div style={{marginTop:14,background:C.card,borderRadius:8,padding:16,color:C.sub,fontSize:13,lineHeight:1.7,whiteSpace:'pre-wrap',maxHeight:350,overflowY:'auto',border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.accent}`}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,marginBottom:8}}>
        <div style={{fontSize:11,color:C.accent,fontWeight:600}}>{resultLabel}</div>
    ...
    try {
      if (mode === 'signup') {
        ...
        if (data?.session?.user) {
          ...
          setMsg('Account created. Check your email to confirm the account, then sign in.');
          setMode('login');
        }
      } else {
      ...
      if (/invalid email/i.test(message)) {
        setErr('That email address is not valid. Remove any spaces and try again.');
      } else if (/email rate limit exceeded/i.test(message)) {
        setErr('Supabase rate-limited signup emails. In Supabase Dashboard, open Authentication → Providers → Email and disable email confirmations for testing, then try again later or use a new email.');
      } else if (/already registered|user already exists|duplicate/i.test(message)) {
        setErr('This email already has an account. Switch to Sign In.');
      } else {
        setErr(message);
      }
    } finally {
  ...
  return <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'"DM Sans",system-ui,sans-serif'}}>
    ...
    <div style={{width:420,position:'relative',zIndex:1}}>
      <div style={{textAlign:'center',marginBottom:40}}>
        ...
        <p style={{color:C.sub,fontSize:14}}>Teacher accounts, AI tools, and file uploads</p>
      </div>
      <Card style={{padding:32}}>
        <div style={{display:'flex',gap:8,marginBottom:18}}>
          <Btn onClick={()=>setMode('login')} variant={mode==='login' ? 'primary' : 'secondary'} size="sm" full>Sign In</Btn>
          <Btn onClick={()=>setMode('signup')} variant={mode==='signup' ? 'primary' : 'secondary'} size="sm" full>Create Account</Btn>
        </div>
        <h2 style={{fontSize:20,fontWeight:700,color:C.text,marginBottom:6,fontFamily:'"Playfair Display",serif'}}>{mode === 'login' ? 'Teacher Login' : 'Create Teacher Account'}</h2>
        <p style={{color:C.sub,fontSize:13,marginBottom:20}}>{mode === 'login' ? 'Sign in to access your dashboard' : 'Create an account with email and password'}</p>

        {mode === 'signup' && (
          <Input label="Full Name" value={fullName} onChange={setFullName} placeholder="Dr. Sarah Ahmed" />
        )}
        <Input label="Email Address" value={email} onChange={setEmail} placeholder="teacher@school.edu" type="email"/>
        ...
        {msg && <div style={{background:`${C.green}18`,border:`1px solid ${C.green}44`,borderRadius:8,padding:'10px 12px',color:C.green,fontSize:12,marginBottom:14}}>{msg}</div>}

        <Btn onClick={handleSubmit} full disabled={load} size="lg">{load ? (mode === 'login' ? 'Signing in...' : 'Creating account...') : (mode === 'login' ? 'Sign In to EduAI' : 'Create Account')}</Btn>
      </Card>
    </div>
  </div>;
}
  ...
  return <div style={{width:240,background:C.surface,borderRight:`1px solid ${C.border}`,display:'flex',flexDirection:'column',height:'100vh',flexShrink:0}}>
    ...
    <div style={{padding:'12px 8px',borderTop:`1px solid ${C.border}`}}>
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',marginBottom:8}}>
        <div style={{width:32,height:32,borderRadius:'50%',background:C.accent,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#000'}}>{(user?.name || user?.email || 'T').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()}</div>
        <div style={{overflow:'hidden'}}><div style={{fontSize:12,fontWeight:600,color:C.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{user?.name || user?.email || 'Teacher'}</div><div style={{fontSize:10,color:C.muted}}>Professor</div></div>
      </div>
...
function TopBar({user,module}) {
  ...
  return <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:'12px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:12}}>
      ...
      <div style={{width:30,height:30,borderRadius:'50%',background:C.accent,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#000'}}>{(user?.name || user?.email || 'T').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()}</div>
    </div>
  </div>;
}
...
function CurriculumModule({courses,setCourses,curriculumAssets,setCurriculumAssets}) {
  ...
  return <div>
    <SectionHead title="Curriculum Design & Update" sub="Manage learning outcomes, syllabi, curriculum files, course documents, and program files" action={<Btn onClick={()=>setAddCourseModal(true)} icon={Plus}>Add Course</Btn>}/>
...
function CurriculumRepository({courses,setCourses,curriculumAssets,setCurriculumAssets}) {
  ...
  const [cloExtracting,setCloExtracting]=useState(false);
  ...
  function resetForm(){ setForm({title:'',category:'Curriculum Document',notes:'',file:null,fileName:'',fileUrl:'',fileType:'',storagePath:'',storageProvider:'local'}); setCloExtractMsg(''); }
  async function onPickFile(file){
    if(!file) return;
    // Always store the file reference first so CLO extraction can work
    ...
    try {
      ...
      // File is still stored locally even if upload fails
      console.warn('File upload to storage failed, file stored locally:', e.message);
    }
  }

  async function extractCLOsFromPdf(file) {
  ...
  async function add(){
    ...
    // Auto-extract CLOs if category is CLO Document, file is PDF, and a specific course is selected
    if (form.category === 'CLO Document' && form.file && fileKind(form.file) === 'pdf' && selectedCourse !== 'all' && setCourses) {
      setCloExtracting(true);
      setCloExtractMsg('Extracting CLOs from PDF...');
      try {
        const text = await extractCLOsFromPdf(form.file);
        if (text && text.length >= 20) {
          setCloExtractMsg('Analyzing CLOs with AI...');
          const cloPrompt = `You are an expert in academic curriculum design. Extract ALL the CLOs (Course Learning Outcomes) from this text. Return ONLY a valid JSON array of objects with "id" (like "CLO1") and "description". No markdown, no explanation.\n\nText:\n${text.substring(0,6000)}`;
          const cloResult = await ai(cloPrompt, 'Extract CLOs. Return ONLY valid JSON arrays.');
          const cleaned = cloResult.replace(/```json\s*/gi,'').replace(/```\s*/g,'').trim();
          const clos = JSON.parse(cleaned);
          if (Array.isArray(clos) && clos.length > 0) {
            const cloStrings = clos.map(c => `${c.id}: ${c.description}`);
            const courseId = +selectedCourse;
            const updatedCourses = courses.map(c => c.id === courseId ? {...c, clos: cloStrings, extractedCLOObjects: clos, cloDocument: form.file.name} : c);
            setCourses(updatedCourses);
            setCloExtractMsg(`✓ ${clos.length} CLOs extracted and saved to course!`);
          } else {
            setCloExtractMsg('No CLOs found in the document.');
          }
        } else {
          setCloExtractMsg('Could not extract text from PDF.');
        }
      } catch (err) {
        setCloExtractMsg('CLO extraction failed: ' + (err.message || 'Unknown error'));
      } finally {
        setCloExtracting(false);
        setTimeout(() => { setCloExtractMsg(''); }, 5000);
      }
    }
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,marginBottom:16}}>
      <div style={{color:C.sub,fontSize:12,maxWidth:680}}>Upload curriculum files, course packs, program documents, CLO files, PLO files, and mapping resources. PDF previews open inline; PPT/PPTX files can be opened from the resource card.</div>
      <Btn onClick={()=>setModal(true)} icon={Plus}>Add Curriculum Resource</Btn>
    </div>
    <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:16}}>
      {courseOptions.map(opt => <button key={opt.id} onClick={()=>setSelectedCourse(opt.id)} style={{padding:'8px 12px',borderRadius:999,border:`1px solid ${selectedCourse===opt.id?C.accent:C.border}`,background:selectedCourse===opt.id?`${C.accent}18`:C.surface,color:selectedCourse===opt.id?C.accent:C.sub,cursor:'pointer',fontSize:12}}>{opt.label}</button>)}
    </div>
    {(cloExtracting||cloExtractMsg)&&<div style={{marginBottom:14,padding:'10px 14px',borderRadius:8,fontSize:12,display:'flex',alignItems:'center',gap:8,background:cloExtractMsg.startsWith('✓')?`${C.green}18`:cloExtractMsg.includes('failed')?`${C.red}18`:`${C.accent}18`,border:`1px solid ${cloExtractMsg.startsWith('✓')?C.green+'44':cloExtractMsg.includes('failed')?C.red+'44':C.accent+'44'}`,color:cloExtractMsg.startsWith('✓')?C.green:cloExtractMsg.includes('failed')?C.red:C.accent}}>
      {cloExtracting&&<Loader size={13}/>}{cloExtractMsg}
    </div>}
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
      {filtered.map(item=><Card key={item.id} style={{padding:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:8}}>
          <Tag label={item.category} color={typeColor[item.category]||C.muted}/>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>open(item)} title="View" style={{background:'none',border:'none',color:C.accent,cursor:'pointer'}}><Eye size={12}/></button>
            <button onClick={()=>window.open(item.fileUrl || '#','_blank')} title="Open" style={{background:'none',border:'none',color:C.green,cursor:'pointer'}}><ChevronRight size={12}/></button>
            <button onClick={()=>del(item.id)} title="Delete" style={{background:'none',border:'none',color:C.red,cursor:'pointer'}}><Trash2 size={12}/></button>
          </div>
        </div>
        <h4 style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:6,lineHeight:1.3}}>{item.title}</h4>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Course/Program: {item.course}</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>File: {item.fileName || 'No file uploaded'}</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Type: {String(item.fileType||'unknown').toUpperCase()}</div>
        <div style={{fontSize:11,color:C.muted}}>Added: {item.date} • {item.size}</div>
      </Card>)}
    </div>
    <Modal open={modal} onClose={()=>{setModal(false);resetForm();}} title="Add Curriculum / Course / Program Resource" width={620}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div style={{marginBottom:14}}>
          <label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Category</label>
          <select value={form.category} onChange={e=>setForm({...form,category:e.target.value})} style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'inherit',outline:'none'}}>
            {categories.map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div style={{marginBottom:14}}>
          <label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Course / Program Scope</label>
          <select value={selectedCourse} onChange={e=>setSelectedCourse(e.target.value)} style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'inherit',outline:'none'}}>
      ...
      <div style={{marginBottom:14}}>
        <label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Upload File (PDF, PPT, PPTX, DOCX, XLSX)</label>
        <input type="file" onChange={e=>onPickFile(e.target.files?.[0])} style={{width:'100%',color:C.text,fontSize:13}}/>
      </div>
      {form.fileName && <div style={{fontSize:11,color:C.muted,marginBottom:14}}>Selected: {form.fileName}</div>}
    ...
    <Modal open={!!viewer} onClose={()=>setViewer(null)} title={viewer?.title || 'Preview'} width={900}>
      {viewer && <div style={{display:'flex',flexDirection:'column',gap:14}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:14,flexWrap:'wrap'}}>
          <div><div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:4}}>{viewer.title}</div><div style={{fontSize:12,color:C.sub}}>{viewer.category} • {viewer.course}</div></div>
          <div style={{display:'flex',gap:10}}><Btn onClick={()=>window.open(viewer.fileUrl || '#','_blank')} variant="secondary" size="sm">Open File</Btn></div>
        </div>
        {viewer.fileType==='pdf' && viewer.fileUrl ? <iframe title={viewer.title} src={viewer.fileUrl} style={{width:'100%',height:'70vh',border:`1px solid ${C.border}`,borderRadius:12,background:'#fff'}} /> : <div style={{padding:16,border:`1px solid ${C.border}`,borderRadius:12,background:C.surface,color:C.sub,fontSize:13,lineHeight:1.7}}><div style={{marginBottom:8}}>Preview is not available inside the browser for this file type.</div><div>Use <strong style={{color:C.text}}>Open File</strong> to view or download the uploaded document.</div></div>}
      </div>}
    </Modal>
  </div>;
}
...
function CLOPLOMapper({courses,setCourses}) {
  ...
  const PLOS = [
    ...
    {id:'PO2',desc:'Identify, formulate, research literature and analyze complex engineering problems reaching substantiated conclusions using first principles of mathematics, natural sciences and engineering sciences with holistic considerations for sustainable development (WK1 to WK4).'},
  ...
  async function handlePdfUpload(file) {
    ...
    try {
      ...
      // Post-process: enforce strict 1:1 — if multiple CLOs got the same PLO, keep only the first
      const usedPLOs = new Set();
      const dedupedMappings = {};
      for (const cloId of Object.keys(mappings)) {
        const ploId = mappings[cloId];
        if (ploId && !usedPLOs.has(ploId)) {
          usedPLOs.add(ploId);
          dedupedMappings[cloId] = ploId;
        }
        // else: skip this CLO (its PLO was already taken by an earlier CLO)
      }
      setCloMappings(dedupedMappings);
      // Save CLOs, mappings, and PDF filename to the selected course
      if (sel && setCourses) {
        const cloStrings = clos.map(c => `${c.id}: ${c.description}`);
        const updatedCourses = courses.map(c => c.id === sel ? {...c, clos: cloStrings, mapping: dedupedMappings, extractedCLOObjects: clos, cloDocument: file.name} : c);
        setCourses(updatedCourses);
      }
      setMappingDone(true);
      setStep('');
  ...
  return <div>
    ...
    {/* Extracted CLOs & PLO list */}
    {extractedCLOs.length>0&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
      <Card>
        <h3 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:4}}>Extracted CLOs from PDF</h3>
        <p style={{fontSize:11,color:C.muted,marginBottom:14}}>{extractedCLOs.length} CLOs found in "{pdfFile?.name}"</p>
        <div style={{maxHeight:420,overflowY:'auto'}}>
          {extractedCLOs.map((clo,i)=><div key={i} style={{padding:'10px 12px',background:C.surface,borderRadius:8,marginBottom:8,fontSize:12,color:C.text,border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.accent}`}}>
            <span style={{color:C.accent,fontWeight:700}}>{clo.id}</span> — {clo.description}
          </div>)}
        </div>
      </Card>
      <Card>
        <h3 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:4}}>Program Outcomes (PO1–PO12)</h3>
        <p style={{fontSize:11,color:C.muted,marginBottom:14}}>Green = mapped by at least one CLO</p>
        <div style={{maxHeight:420,overflowY:'auto'}}>
          {PLOS.map(plo=>{const mapped=mappedPLOIds.has(plo.id);return <div key={plo.id} style={{padding:'10px 12px',background:mapped?`${C.green}15`:C.surface,borderRadius:8,marginBottom:8,fontSize:11,color:mapped?C.text:C.muted,border:`1px solid ${mapped?C.green+'44':C.border}`,display:'flex',gap:8,alignItems:'flex-start'}}>
    ...
    {/* Mapping Matrix */}
    {mappingDone&&<Card style={{marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
        <h3 style={{fontSize:14,fontWeight:600,color:C.text}}>CLO → PLO Mapping Matrix</h3>
        <Tag label={`${extractedCLOs.length} CLOs × ${PLOS.length} POs`} color={C.accent}/>
      </div>
      <div style={{overflowX:'auto'}}>
        <table style={{borderCollapse:'collapse',fontSize:11,width:'100%'}}>
          <thead><tr>
            <th style={{padding:'8px 12px',textAlign:'left',color:C.muted,borderBottom:`1px solid ${C.border}`,minWidth:100,position:'sticky',left:0,background:C.card,zIndex:1}}>CLO</th>
            {PLOS.map(p=><th key={p.id} style={{padding:'8px 6px',color:C.muted,borderBottom:`1px solid ${C.border}`,textAlign:'center',whiteSpace:'nowrap',fontSize:10}}>{p.id}</th>)}
          </tr></thead>
          <tbody>{extractedCLOs.map((clo,ci)=><tr key={ci} style={{borderBottom:`1px solid ${C.border}`,background:ci%2===0?C.card:C.surface}}>
            <td style={{padding:'8px 12px',color:C.text,fontWeight:500,position:'sticky',left:0,background:ci%2===0?C.card:C.surface,zIndex:1,whiteSpace:'nowrap'}} title={clo.description}>{clo.id}</td>
            {PLOS.map(p=>{const mapped=cloMappings[clo.id]===p.id;return <td key={p.id} style={{textAlign:'center',padding:'8px 6px'}}>{mapped?<div style={{width:20,height:20,background:C.green,borderRadius:4,display:'inline-flex',alignItems:'center',justifyContent:'center'}}><CheckCircle size={11} color="#fff"/></div>:<div style={{width:20,height:20,background:C.border,borderRadius:4,display:'inline-block',opacity:0.3}}/>}</td>;})}
          </tr>)}</tbody>
        </table>
      </div>
    </Card>}
    ...
    {mappingDone&&<Card>
      <h3 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:14}}>Detailed CLO → PLO Mapping</h3>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        {extractedCLOs.map(clo=>{const poId=cloMappings[clo.id];const po=PLOS.find(p=>p.id===poId);return <div key={clo.id} style={{padding:14,background:C.surface,borderRadius:10,border:`1px solid ${C.border}`}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <Tag label={clo.id} color={C.accent}/>
            <ChevronRight size={12} color={C.muted}/>
            <Tag label={poId||'N/A'} color={C.green}/>
          </div>
          <div style={{fontSize:11,color:C.sub,lineHeight:1.5,marginBottom:6}}><strong style={{color:C.text}}>CLO:</strong> {clo.description}</div>
          {po&&<div style={{fontSize:11,color:C.muted,lineHeight:1.5}}><strong style={{color:C.sub}}>PLO:</strong> {po.desc.length>150?po.desc.substring(0,150)+'...':po.desc}</div>}
        </div>;})}
      </div>
    </Card>}

    {/* Existing course CLO-PLO mapping (fallback for courses with pre-defined data) */}
    {course&&!mappingDone&&extractedCLOs.length===0&&<div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
        <Card>
          ...
          {(course.clos||[]).length > 0 ? (course.clos||[]).map((clo,i)=>{
            ...
            return <div key={i} style={{padding:'10px 12px',background:C.surface,borderRadius:8,marginBottom:8,fontSize:12,color:C.text,border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.accent}`}}><span style={{color:C.accent,fontWeight:700}}>CLO{i+1}</span> — {cloText}</div>;
          }) : <div style={{padding:16,background:C.surface,borderRadius:8,border:`1px dashed ${C.border}`,textAlign:'center'}}>
            <p style={{fontSize:12,color:C.muted,marginBottom:4}}>No CLOs uploaded yet</p>
            <p style={{fontSize:11,color:C.muted}}>Upload a CLO PDF above or go to Course & Program Uploads to add a CLO Document</p>
          </div>}
        </Card>
        <Card>
          <h3 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:14}}>Program Outcomes (PO1–PO12)</h3>
          {PLOS.map(plo=><div key={plo.id} style={{padding:'10px 12px',background:C.surface,borderRadius:8,marginBottom:8,fontSize:11,color:C.muted,border:`1px solid ${C.border}`,display:'flex',gap:8,alignItems:'flex-start'}}>
  ...
  return <div>
    ...
    {course&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
      <Card><h4 style={{fontSize:13,fontWeight:600,color:C.accent,marginBottom:12}}>Course Information</h4>
        ...
        :<div style={{fontSize:12,color:C.sub,display:'flex',flexDirection:'column',gap:8}}><div><span style={{color:C.muted}}>Code: </span><strong style={{color:C.text}}>{course.code}</strong></div><div><span style={{color:C.muted}}>Name: </span><strong style={{color:C.text}}>{course.name}</strong></div><div><span style={{color:C.muted}}>Credits: </span><strong style={{color:C.text}}>{course.credits} credit hours</strong></div><div><span style={{color:C.muted}}>Semester: </span><strong style={{color:C.text}}>{course.semester}</strong></div><div><span style={{color:C.muted}}>Students: </span><strong style={{color:C.text}}>{course.students}</strong></div>{course.cloDocument&&<div><span style={{color:C.muted}}>CLO Document: </span><strong style={{color:C.green}}>{course.cloDocument}</strong></div>}</div>}
      </Card>
      <Card><h4 style={{fontSize:13,fontWeight:600,color:C.accent,marginBottom:12}}>Course Learning Outcomes</h4>
        {courseClos.length > 0 ? courseClos.map((clo,i)=>{
          ...
          return <div key={i} style={{padding:'8px 10px',background:C.surface,borderRadius:6,marginBottom:6,fontSize:12,color:C.sub,border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.accent}`}}>{cloText}</div>;
        }) : <div style={{padding:16,background:C.surface,borderRadius:8,border:`1px dashed ${C.border}`,textAlign:'center'}}>
          <p style={{fontSize:12,color:C.muted,marginBottom:4}}>No CLOs uploaded yet</p>
          <p style={{fontSize:11,color:C.muted}}>Go to CLO-PLO Mapping tab to upload a CLO document for this course</p>
        </div>}
      </Card>
...
function ResourceLib({materials,setMaterials}) {
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,marginBottom:16}}>
      <div style={{color:C.sub,fontSize:12,maxWidth:680}}>Upload lecture slides, PDFs, notes, and other teaching resources. Click a card to preview a PDF or open the uploaded file.</div>
      <Btn onClick={()=>setModal(true)} icon={Plus}>Add Resource</Btn>
    </div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
      {materials.map(m=><Card key={m.id} style={{padding:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:8}}>
          <Tag label={m.type} color={typeColor[m.type]||C.muted}/>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>open(m)} title="View" style={{background:'none',border:'none',color:C.accent,cursor:'pointer'}}><Eye size={12}/></button>
            <button onClick={()=>m.fileUrl ? window.open(m.fileUrl,'_blank') : null} title="Open" disabled={!m.fileUrl} style={{background:'none',border:'none',color:m.fileUrl?C.green:C.muted,cursor:m.fileUrl?'pointer':'not-allowed'}}><ChevronRight size={12}/></button>
            <button onClick={()=>del(m.id)} title="Delete" style={{background:'none',border:'none',color:C.red,cursor:'pointer'}}><Trash2 size={12}/></button>
          </div>
        </div>
        <h4 style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:6,lineHeight:1.3}}>{m.title}</h4>
        <div style={{fontSize:11,color:C.muted}}>Course: {m.course || 'N/A'}</div>
        <div style={{fontSize:11,color:C.muted,marginTop:4}}>File: {m.fileName || 'No file uploaded'}</div>
        <div style={{fontSize:11,color:C.muted,marginTop:4}}>Type: {String(m.fileType || 'unknown').toUpperCase()}</div>
        <div style={{fontSize:11,color:C.muted,marginTop:4}}>Added: {m.date} • {m.size}</div>
        {m.notes && <div style={{fontSize:11,color:C.sub,marginTop:8,lineHeight:1.5}}>{m.notes}</div>}
      </Card>)}
    </div>
    <Modal open={modal} onClose={()=>{setModal(false);resetForm();}} title="Add Teaching Resource" width={560}>
      ...
      <div style={{marginBottom:14}}>
        ...
        <select value={form.type} onChange={e=>setForm({...form,type:e.target.value})} style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'inherit',outline:'none'}}>
      ...
      <div style={{marginBottom:14}}>
        ...
        <input
          ...
          style={{width:'100%',color:C.text,fontSize:13}}
        />
      </div>
      {form.fileName && <div style={{fontSize:11,color:C.muted,marginBottom:14}}>Selected: {form.fileName} • {String(form.fileType||'unknown').toUpperCase()}</div>}
      <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
        <Btn onClick={()=>{setModal(false);resetForm();}} variant="ghost">Cancel</Btn>
        <Btn onClick={add} icon={Plus}>Add Resource</Btn>
      </div>
    ...
    <Modal open={!!viewer} onClose={()=>setViewer(null)} title={viewer?.title || 'Preview'} width={900}>
      {viewer && <div style={{display:'flex',flexDirection:'column',gap:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:14,flexWrap:'wrap'}}>
          <div>
            <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:4}}>{viewer.title}</div>
            <div style={{fontSize:12,color:C.sub}}>{viewer.type} • {viewer.course || 'No course assigned'}</div>
            <div style={{fontSize:11,color:C.muted,marginTop:4}}>File: {viewer.fileName || 'No file uploaded'}</div>
          </div>
        ...
        {viewer.fileType==='pdf' && viewer.fileUrl ? (
          ...
          <div style={{padding:16,border:`1px solid ${C.border}`,borderRadius:12,background:C.surface,color:C.sub,fontSize:13,lineHeight:1.7}}>
            <div style={{marginBottom:8}}>Preview is not available inside the browser for this file type.</div>
            <div>Use <strong style={{color:C.text}}>Open File</strong> to view or download the uploaded document.</div>
            {viewer.notes && <div style={{marginTop:12,whiteSpace:'pre-wrap'}}>{viewer.notes}</div>}
          </div>
        )}
      </div>}
    </Modal>
...
// ===================== CAPSTONE MODULE =====================
function CapstoneModule({projects,setProjects,capstoneSubmissions,setCapstoneSubmissions}) {
  const [tab,setTab]=useState('projects');
  const tabs=[{id:'projects',icon:Database,label:'Project Tracker'},{id:'milestones',icon:Calendar,label:'Milestones'},{id:'uploads',icon:Upload,label:'Student Uploads'},{id:'screen',icon:Eye,label:'AI Proposal Screener'},{id:'eval',icon:CheckSquare,label:'Evaluation & Rubrics'},{id:'reports',icon:FileText,label:'Reports'}];
  return <div><SectionHead title="Capstone Supervision & Evaluation" sub="Track projects, screen proposals, support uploads, and evaluate student capstones"/><TabBar tabs={tabs} active={tab} setActive={setTab}/>
...
${sourceText || 'No extracted text was provided. Use the file metadata and title to give a cautious, limited-scope screening.'}`; const report = await ai(prompt, 'You are an academic originality screening assistant. Never state certainty beyond the evidence provided. Do not pretend to run a database similarity scan. Clearly label the output as a heuristic screening.', item.provider || 'groq'); setSubmissions(submissions.map(s=>s.id===item.id?{...s,report,plagiarismStatus: report.toLowerCase().includes('high') ? 'High risk' : report.toLowerCase().includes('medium') ? 'Medium risk' : 'Checked'}:s)); setChecking(null); }
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,marginBottom:16}}>
      <div style={{color:C.sub,fontSize:12,maxWidth:720}}>Students can upload their capstone project files here. The AI plagiarism/originality screen works best when an abstract or extracted text is also provided.</div>
      <Btn onClick={()=>setModal(true)} icon={Upload}>Upload Project</Btn>
    </div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
      {submissions.map(item=><Card key={item.id} style={{padding:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:8}}>
          ...
          <div style={{display:'flex',gap:8}}><button onClick={()=>open(item)} title="View" style={{background:'none',border:'none',color:C.accent,cursor:'pointer'}}><Eye size={12}/></button><button onClick={()=>del(item.id)} title="Delete" style={{background:'none',border:'none',color:C.red,cursor:'pointer'}}><Trash2 size={12}/></button></div>
        </div>
        <h4 style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:6,lineHeight:1.3}}>{item.title || 'Untitled Project'}</h4>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Student: {item.student || 'N/A'}</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Supervisor: {item.supervisor || 'N/A'}</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:4}}>File: {item.fileName || 'No file'}</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:8}}>Uploaded: {item.date} • {item.size}</div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Btn onClick={()=>analyze(item)} size="sm" variant="secondary" icon={Brain}>{checking===item.id?'Checking...':'Check Plagiarism'}</Btn></div>
      </Card>)}
    </div>
    <Modal open={modal} onClose={()=>{setModal(false);resetForm();}} title="Upload Student Capstone Project" width={700}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Input label="Student Name" value={form.student} onChange={v=>setForm({...form,student:v})} placeholder="Student full name"/><Input label="Supervisor" value={form.supervisor} onChange={v=>setForm({...form,supervisor:v})} placeholder="Supervisor name"/></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
        <div><label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>Upload Project File (PDF, PPT, PPTX, DOCX)</label><input type="file" onChange={e=>onPickFile(e.target.files?.[0])} style={{width:'100%',color:C.text,fontSize:13}}/></div>
        <div><label style={{display:'block',fontSize:12,color:C.sub,marginBottom:6,fontWeight:600}}>AI Provider</label><select value={form.provider} onChange={e=>setForm({...form,provider:e.target.value})} style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'inherit',outline:'none'}}><option value="groq">Groq</option><option value="gemini">Gemini</option><option value="openrouter">OpenRouter</option></select></div>
      </div>
      {form.fileName && <div style={{fontSize:11,color:C.muted,marginBottom:14}}>Selected: {form.fileName}</div>}
      <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><Btn onClick={()=>{setModal(false);resetForm();}} variant="ghost">Cancel</Btn><Btn onClick={add} icon={Upload}>Save Submission</Btn></div>
    </Modal>
    <Modal open={!!viewer} onClose={()=>setViewer(null)} title={viewer?.title || 'Submission'} width={900}>
      {viewer && <div style={{display:'flex',flexDirection:'column',gap:14}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12,flexWrap:'wrap'}}><div><div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:4}}>{viewer.title}</div><div style={{fontSize:12,color:C.sub}}>{viewer.student} • {viewer.supervisor}</div></div><div style={{display:'flex',gap:10}}><Btn onClick={()=>window.open(viewer.fileUrl || '#','_blank')} variant="secondary" size="sm">Open File</Btn></div></div><div style={{padding:16,border:`1px solid ${C.border}`,borderRadius:12,background:C.surface,color:C.sub,fontSize:13,lineHeight:1.7}}><div><strong style={{color:C.text}}>Abstract / Text:</strong></div><div style={{whiteSpace:'pre-wrap',marginTop:6}}>{viewer.abstract || 'No abstract/text provided.'}</div><div style={{marginTop:12}}><strong style={{color:C.text}}>AI Report:</strong></div><div style={{whiteSpace:'pre-wrap',marginTop:6}}>{viewer.report || 'No plagiarism/originality check has been run yet.'}</div></div></div>}
    </Modal>
  </div>;
}
...
export default function EduAI() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProjects(p);
      setMaterials(m);
      setCurriculumAssets(ca);
      setCapstoneSubmissions(cs);
  ...
  if (authLoading) {
    return <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:C.bg,color:C.text,fontFamily:'"DM Sans",system-ui,sans-serif'}}>
  ...
  if(page==='login')return <LoginPage onAuthSuccess={(authUser)=>{setUser({name:getDisplayName(authUser), email:authUser?.email, dept:'CSE', id:authUser?.id || 'demo'});setPage('app');}}/>;
  return <div style={{display:'flex',height:'100vh',background:C.bg,color:C.text,fontFamily:'"DM Sans",system-ui,sans-serif',overflow:'hidden'}}>
    ...
    <Sidebar module={module} setModule={setModule} onLogout={handleLogout} user={user}/>
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <main style={{flex:1,overflowY:'auto',padding:'24px 28px'}}>
        ...
        {module==='capstone'&&<CapstoneModule projects={projects} setProjects={updProjects} capstoneSubmissions={capstoneSubmissions} setCapstoneSubmissions={updCapstoneSubmissions}/>}
      </main>
    </div>
  </div>;
}
