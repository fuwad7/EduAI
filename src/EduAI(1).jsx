...
async function uploadManagedFile(file, folder='uploads') {
  if (!file) return { fileUrl: '', storagePath: '', storageProvider: 'local' };
  ...
  if (HAS_SUPABASE) {
    try {
      ...
      console.warn('Supabase upload failed, falling back to local data URL.', err);
    }
  }

  return { fileUrl: await fileToDataUrl(file), storagePath: '', storageProvider: 'local' };
}
  ...
  async function handleSubmit() {
    ...
    if (!HAS_SUPABASE) {
      if (email === 'teacher@eduai.com' && pass === 'demo123') {
        onAuthSuccess({ id: 'demo', email, user_metadata: { full_name: fullName || 'Dr. Sarah Ahmed' } });
        return;
      }
    ...
    <div style={{width:420,position:'relative',zIndex:1}}>
      ...
      <Card style={{padding:32}}>
        ...
        <div style={{marginTop:20,background:C.surface,borderRadius:8,padding:14,border:`1px dashed ${C.border}`}}>
          <div style={{fontSize:11,color:C.accent,fontWeight:600,marginBottom:6}}>SETUP NOTE</div>
          <div style={{fontSize:12,color:C.sub,lineHeight:1.6}}>
            Connect Supabase Auth and create a storage bucket named <strong style={{color:C.text}}>eduai-files</strong> for uploaded PDFs and documents.
          </div>
          {!HAS_SUPABASE && <Btn onClick={()=>{setMode('login');setEmail('teacher@eduai.com');setPass('demo123');}} variant="ghost" size="sm" style={{marginTop:8}}>Use Demo Login</Btn>}
        </div>
      </Card>
    </div>
  </div>;
